    ...
    <Card className='container mx-auto flex items-center h-screen -z-10'>
      <div className='flex w-full mt-10'>
        <div className='flex flex-col w-1/2 items-center gap-5'>
          ...
          <Rating className='bg-slate-100 px-10 py-5 rounded-lg'>
            <Rating.Star />
            <Rating.Star />
            <Rating.Star />
            <Rating.Star />
        ...
        <div className='flex flex-col justify-between w-1/2 px-5'>
          <div>
            ...
            <span className='py-4 font-semibold'>${price}</span>
            <p className='py-4 text-sm'>{description}</p>
          </div>
          <button className="rounded-lg bg-cyan-700 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-cyan-800 focus:outline-none focus:ring-4 focus:ring-cyan-300 dark:bg-cyan-600 dark:hover:bg-cyan-700 dark:focus:ring-cyan-800  group-hover:opacity-100" onClick={()=> addItem(productItem, productItem.id)}>Add to Cart</button>
        </div>
      </div>
    </Card>
  )
